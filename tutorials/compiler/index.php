...
<p><em>Note: This tutorial was written for version 0.6 of the Shadow compiler and may not be entirely accurate for previous or future versions.</em></p>
<p>The Shadow compiler is required in order to transform written Shadow programs into their executable counterparts. It provides a number of helpful features, including descriptive error-messages and a simple, automated build system. The compiler, along with detailed installation instructions, is available on the <a href="../../downloads" title="Downloads">Downloads</a> page.</p>
<h3>Getting Started</h3>
<p>Like many utilities, the Shadow compiler is operated via the command-line. Access to the command-line varies by platform, but is usually available through a terminal emulator. Running the install script included with the Shadow compiler adds its location to the system's PATH variable, allowing it to be run from any directory with the command <code>shadowc</code>.</p>
<p>For simple programs, compilation can be invoked with the following command:</p>
<pre>
shadowc Main.shadow
</pre>
<p>In this case, <code>Main.shadow</code> is a Shadow source file containing a main method. This main method will become the entry point of the program in the resulting executable. The compiler will automatically attempt to resolve any dependencies, both locally and within the standard library.</p>
<p>By default, the resulting executable will bear the same name as the source file (in this case <code>Main.exe</code> on Windows and <code>Main</code> on other systems). The executable name can be specified using the <code>-o</code> option:</p>
<pre>
shadowc Main.shadow -o UsefulProgram.exe
</pre>
<h3>Additional Options</h3>
<p>When run with the option <code>-h</code> (or <code>--help</code>), the compiler will print a list of all available options and their descriptions. This will also occur if an invalid option or argument is specified, along with a corresponding error message. For reference, the help printout is reproduced below:</p>
<pre>
usage: shadowc &lt;mainSource.shadow&gt; [-o &lt;output&gt;] [-c &lt;config.xml&gt;]
 ...
 -f,--force-recompile       Recompiles all source files, even if
                            unnecessary
...
<li><code>&lt;include&gt;</code> - Used to define additional search paths for resolving dependencies (<code>import</code> statements). If any include paths are specified, the path <code>&lt;include&gt;.&lt;/include&gt;</code> must also be. <em>(optional)</em></li>
</ul>
...
<p>In the example above, the user has explicitly specified some platform and directory information. Within the <code>&lt;shadow&gt;</code> tag, the <code>os=&quot;Linux&quot;</code> attribute ensures that the compiler will use Linux-compliant system calls for standard library functions. The attribute <code>arch=&quot;64&quot;</code> ensures that 64-bit addressing is used. Although the <code>target</code> attribute seems to contain redundant information, it represents a special set of information for use by the compiler's LLVM backend (specifically, the last stages of compilation which output platform-specific machine code). See the section on <a href="#triples">LLVM Target Triples</a> for more information.</p>
<p>The <code>&lt;system&gt;</code> tag is used to explicitly specify that the Shadow standard library is located in <code>/home/dave/standardlibs/</code>. Within this directory, the compiler looks for the directory <code>shadow/</code> containing the libraries in question. The tag <code>&lt;import&gt;.&lt;/import&gt;</code> tells the compiler to resolve import statements by searching directories relative to the file being compiled. This tag must always be specified <strong>if</strong> any other include paths are specified, or the standard libraries (and presumably most user programs) will fail to resolve dependencies. Additional paths, such as the one specified in <code>&lt;import&gt;/usr/local/lib/extralibs&lt;/import&gt;</code> will also be searched when resolving dependencies.</p>
<h4>Configuration for Microsoft Windows</h4>
<p>The configuration file below describes the currently necessary platform attributes when compiling on (and for) Microsoft Windows. Because MinGW does not support 64-bit compilation, it is important to prevent the compiler from attempting otherwise.</p>
<pre class="prettyprint lang-shadow">
&lt;?xml version=&quot;1.0&quot; encoding=&quot;UTF-8&quot;?&gt;
...
<p>The canonical form of LLVM target triple is as follows: Either <code>Architecture-Vendor-OperatingSystem</code> or <code>Architecture-Vendor-OperatingSystem-Environment</code>.</p>
<ul>
...
<p><em>Note: <code>unknown</code> is a valid entry in any of these fields. The most critical fields to fill are those for architecture and operating system.</em></p>
...
<p>All LLVM tools are capable of automatically detecting the correct triple for a given platform. If LLVM is properly installed, the command <code>llc --version</code> will display information including the default triple. A compiled version of this tool comes with the Windows installation of Shadow, and can be run from the associated directory. However, the Windows platform currently has limitations - see the above section for details.</p>
		</div>
	</div>
	<div id="push"></div>
</div>		
