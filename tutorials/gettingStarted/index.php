<?php
	$path = '../../';
	$subtitle = 'Tutorials<br/>Getting Started - &quot;Hello, World!&quot;';
	include $path . 'start.inc';	
?>
	<meta name="author" content="Team Shadow" />
	<meta name="description" content="Getting Started - &quot;Hello, World!&quot; - Shadow Tutorials">
...
<p>The very first line in the program serves as a <em>comment</em>. Comments allow an author to annotate code with relevant information. In practice, comments are used to describe the function of a segment of code or to provide important metadata about the program. There are two different rules which define comments:</p>
<ul>
...
<p>To import all classes within a particular package, you can leave off a particular class at the end. If access to the entire contents of io was desired, the following statement would be used:</p>
<pre class="prettyprint lang-shadow">
import shadow:io;
</pre>
...
<p>Aside from being a member method of <code>HelloWorld</code>, <code>main()</code> serves a unique purpose. In order to compile an executable program, a main() method must be present somewhere within the program. The execution of a program always begins within its <code>main()</code> method, from which other methods may be called. Put simply, it's the starting point of the program.</p>
<h3>Printing text</h3>
<pre class="prettyprint lang-shadow">
Console:instance.printLine(&quot;Hello, world!&quot;);
</pre>
...
<p>The <code>printLine(&quot;Hello, world!&quot;)</code> portion of this line calls a method named <code>printLine()</code> with the parameter <code>&quot;Hello, World&quot;</code>. In turn, this causes the text <code>&quot;Hello, World&quot;</code> to be printed to the screen. But what is the purpose of the <code>Console:instance</code> portion?</p>
<p>Once again, the syntax in this statement represents a special case. It is worth remembering that methods must always be members of a surrounding class. In addition, methods can only be called from an existing <em>instance</em> of a given class, known as an object. An object must be created prior to calling any member methods.</p>
<p><code>Console</code>, however, is a special kind of class called a singleton. This means that only one <code>Console</code> object can exist within the entire program (in reality, within an individual thread of the program). Normally, an object is created using the <code>create</code> keyword. However, a singleton is created the first time the <code>instance</code> keyword is used. Any later uses of <code>instance</code> will retrieve the existing object. In this case, the <code>Console:instance</code> command gives us access to the <code>Console</code> object which has the ability to print out information using its <code>printLine()</code> method described above.</p>
		</div>
	</div>
	<div id="push"></div>
</div>		
